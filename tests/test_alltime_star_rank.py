...
class FakeSearchClient:
    ...
    def search_repositories(self, query: str, *, sort: str, pages: int) -> list[dict[str, Any]]:
        self.queries.append((query, sort, pages))
...
class AllTimeStarRankTests(unittest.TestCase):
    ...
    def test_entries_are_deduplicated_filtered_and_sorted_by_total_stars(self) -> None:
        ...
        entries = build_alltime_entries(items, top_limit=1000)
    ...
    def test_outputs_satisfy_public_schemas_and_search_contract(self) -> None:
        client = FakeSearchClient([search_item(index + 1, 900_000 - index * 10) for index in range(25)])
        ...
        self.assertEqual(client.queries, [("stars:>=10000", "stars", 10)])
        self.assertEqual(board["entry_count"], 25)
        self.assertEqual(board["source_metrics"]["search_result_count"], 25)
    ...
    def test_run_writes_validated_board_next_to_the_other_public_data(self) -> None:
        client = FakeSearchClient([search_item(index + 1, 700_000 - index) for index in range(12)])
