...
function cardSvg(ranking, label) {
  const rows = ranking.entries.slice(0, 5).map((entry, index) => {
    ...
    const gained = `${entry.stars_gained >= 0 ? '+' : ''}${entry.stars_gained.toLocaleString('en-US')}`;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
    ...
    <text x="88" y="178" font-family="sans-serif" font-size="56" font-weight="850" letter-spacing="-2" fill="#171814">${escapeXml(ranking.date)} · STAR GROWTH</text>
    <text x="90" y="222" font-family="sans-serif" font-size="20" fill="#68685e">CANDIDATE POOL SIGNAL · ${ranking.eligible_count.toLocaleString('en-US')} COMPARABLE REPOSITORIES · UTC+8</text>
    <g font-family="Arial,sans-serif">${rows}</g>
    <text x="90" y="595" font-family="sans-serif" font-size="18" fill="#68685e">GITHUB PUBLIC API · VALID MIDNIGHT SNAPSHOTS · NO ZERO-FILL · NO INTERPOLATION</text>
  </svg>`;
}

parentPort.on('message', async ({ ranking, label, output, fontFile }) => {
  try {
    const rendered = new Resvg(cardSvg(ranking, label), {
