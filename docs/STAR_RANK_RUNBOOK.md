...
完整启用步骤、权限和故障边界见[独立运行器](../ops-worker/README.md)。默认观察模式，恢复与Issue开关关闭。GitHub仍保存源码、数据和部署站点；Cloudflare负责独立检查、受限补调度与R2存储。备份工作流每日北京时间03:30运行，独立并发组、读取固定提交、不阻塞采集。未配置R2密钥/地址时明确失败，不声称已有备份。
