...
- 主任务在北京时间 00:20 首次调度，并在 00:50、01:20 提供两次同日幂等恢复机会；目标是在 03:15 一致性检查前完成。
- 新建或替换的生产快照必须位于北京时间 `[00:00, 03:00)`。
- 相邻快照日期必须连续，实际间隔必须为 21–27 小时。
- 同一自然日普通重跑复用已有有效快照，不改变统计窗口，也不重复调用 GitHub API、中文本地化或分类模型。
- 缺失或无效日期不补采、不补零、不插值；周期榜等待重新形成足够的连续窗口。
- 03:15 watchdog 检查数据分支与公开网站；数据超过 36 小时、今日快照缺失、采集失败、部署失败或两端内容不一致时，创建或更新 `[开源星榜] 每日任务故障`。只有数据分支与公开站点均为今日有效版本且内容一致时才自动关闭。
...
- Python 数据测试、连续两周期离线演练、Node 测试、Astro 检查、静态构建、Playwright/axe、可复现构建、依赖审计和 CodeQL 全部通过。
