...
- 接口需要支持非流式 Chat Completions、`response_format: json_schema`、`finish_reason: stop`。不会自动降级为自由文本、修补截断 JSON 或发送到备用服务。最多两次请求，401/403/429 与重定向立即停止；错误日志不回显响应体、提示词或密钥。
