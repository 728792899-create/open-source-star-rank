...
每次在线翻译/分类结束后写入 `public/enrichment-runs/history/<run-id>.json`，并更新 `latest.json` 对应类型。记录包括实际请求次数（含传输重试）、接受/失败项目、固定失败原因、已观察输入/输出及缓存 Token。失败的模型内容只用于同批第二次纠错，不持久化原文；服务错误、提示词与密钥不进入记录。每次新运行使用新 ID，重用 client 也会重置计数；缓存命中且无需请求的在线运行可记录 0 次。离线/public-only 不生成记录，历史和最近指针必须一致，Schema 校验失败不发布。

状态页和 `operations.json` 暴露最近一次记录，Actions 摘要链接记录路径；历史文件用于后续按日期汇总，但本步尚未提供月度报表或预算熔断。不要把 observed_tokens 当完整用量：`unknown_usage_requests > 0` 表示仍有未知成本。

`tools/enrichment_metrics.py` 的价格表来自 [DeepSeek 官方价格](https://api-docs.deepseek.com/quick_start/pricing)，核对日期 2026-09-24，有效期至 2026-10-24，仅支持官方接口的 `deepseek-flash`。按完整缓存命中、未命中与输出用量计算高峰/低峰美元区间，不推断中国节假日或账户账单。价格过期、缓存拆分缺失、任一请求缺失用量或其他模型时费用为 null；不能为了显示金额填零。更新价格必须重新核对官方来源并更新日期、费率与测试，不影响正常采样或补全继续运行。


## 中文补全 v2 的摘要边界

短描述保留关键技术缩写、型号、版本和数字，普通英文大写词按语义翻译。超过 600 字符的描述，或必须保留的标识总长超过 120 字符，改为核心用途摘要，允许省略长目录、命令和附带示例；出现的数字和带数字型号仍检查源文依据，但不宣称完整自动事实核验。完整原始仓库名与源描述仍由站点保留。

v2 不修改 repository source hash 或重试队列：成功缓存不重新生成，失败仍等正常退避资格。仅新成功生成时更新目录的 prompt_version；目录可能包含保留的历史行，该字段不是逐行生成版本。Schema 和站点读取器兼容 v1/v2；发布 v2 数据后，回滚到仅识别 v1 的源码必须同时恢复匹配数据，不能只回滚代码。
